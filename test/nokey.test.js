...
function testSig(){
  ...
  /* 조표를 셀 으뜸음 — 오선보(renderStaff)도 이 규칙을 함께 쓴다 */
  eq('sigPc: 단조는 나란한 장조로(+3반음)',sigPc(7,true),10);
  eq('sigPc: 장조는 그대로',sigPc(7,false),7);

  /* 한 소리를 #조·b조 두 가지로 적을 수 있는 자리는 조표가 적은 쪽을 쓴다.
     개수까지 같은 F#/Gb 만 #/b 취향으로 가른다 — 취향과 어긋나면 음마다 임시표가 붙는다. */
  eq('Db(b5) 를 쓴다 — C#장조 #7개가 아니라',keySig(1,'flat'),['b',5]);
  eq('B(#5) 를 쓴다 — Cb장조 b7개가 아니라',keySig(11,'sharp'),['#',5]);
  eq('F#/Gb 는 6개로 같다 → b 취향이면 Gb',keySig(6,'flat'),['b',6]);
  eq('F#/Gb 는 6개로 같다 → # 취향이면 F#',keySig(6,'sharp'),['#',6]);
  eq('취향을 안 주면 PREF 를 따른다(Gb)',keySig(6),['b',6]);
}
