...
function drawR(notes,o){
  ...
  /* ★ headR = 머리의 세로 반지름(칸 단위). 실제 조판은 **0.5**다 — 머리가 정확히
     한 칸 높이라 칸에 놓이면 위아래 줄에 닿는다. 0.44 로 그리면 닿지 않아서
     **v63에서 잡은 버그가 재현되지 않는다** — 그래서 예전 테스트가 전부 통과하면서도
     칸에 놓인 2분·온음표가 통째로 안 보이고 있었다. 아래 CASES 의 headR:0.5 줄들이 그걸 지킨다. */
  const head=(x,cy,hollow)=>{ g.save(); g.translate(x,cy); g.rotate(-0.32);
    g.beginPath(); g.ellipse(0,0,sp*0.62,sp*(o.headR||0.44),0,0,Math.PI*2);
  ...
  /* 덧줄(geom.test.js 의 draw() 와 같은 그리기) — v73 버그(76번 항목)는 **여기가 없어서**
     합성 그림으로 재현되지 않았다. 오선 밖 음은 기둥이 반드시 덧줄을 지나가므로
     (기둥 길이가 3.5칸이라 덧줄 1~2개 정도는 늘 그 안에 든다), 덧줄을 실제로 그려야만
     "덧줄 자체가 빔 한 겹으로 잘못 세이는가"를 검사할 수 있다. */
  const ledger=(x,st)=>{ if(st>=0&&st<=8)return;
    ...
    for(let k=1;k<=n;k++){ const ly=st<0?bot+k*sp:y0-k*sp;
      ...
      g.moveTo(x-sp*0.95,ly); g.lineTo(x+sp*0.95,ly); g.stroke(); } };
...
const CASES=[
  ...
  /* ★★ v63 — 머리를 **실제 크기(한 칸)**로 그렸을 때. 칸에 놓인 속 빈 머리는
     위아래 줄에 닿고, 닿은 자리의 테두리가 줄과 함께 지워져 구멍이 터진다.
     그러면 그 음이 **통째로 사라졌다**(실측: 4개 중 0개 검출).
     사용자 신고 *"파, 미 인데 파만 나왔다(파가 4박자로 되어 있음)"* 가 이것이다. */
  ['칸에 놓인 2분음표(실제 크기 머리)', [N(-1,2),N(1,2),N(3,2),N(5,2),N(7,2)], {headR:0.5}],
  ['칸에 놓인 온음표(실제 크기 머리)', [N(1,4),N(3,4),N(5,4),N(7,4),N(9,4)], {headR:0.5}],
  ['줄 위에 놓인 2분음표(실제 크기 머리)', [N(0,2),N(2,2),N(4,2),N(6,2),N(8,2)], {headR:0.5}],
  /* ★ 그리고 그 고침이 **8분·16분을 건드리면 안 된다** — 지운 줄을 되살리면 꼬리·기둥·줄이
     두르는 주머니가 속 빈 머리로 잡혀 8분음표가 4분·2분음표로 둔갑한다(실측). */
  ['칸에 놓인 8분음표(실제 크기 머리)', [N(-1,0.5),N(1,0.5),N(3,0.5),N(5,0.5),N(7,0.5)], {headR:0.5}],
  ['칸에 놓인 16분음표(실제 크기 머리)', [N(1,0.25),N(3,0.25),N(5,0.25),N(7,0.25)], {headR:0.5}],
  /* ★★★★★★★ v73(CLAUDE.md 76번 항목) — 오선 아래 덧줄을 지나는 기둥이 그 덧줄 자체를
     "진짜 빔·꼬리 한 겹 더"로 잘못 셌다. 이 케이스들은 위 `ledger()` 가 실제로 덧줄을
     그려야만 재현된다 — 예전 drawR 은 덧줄을 안 그려서 이 버그를 통과시키고 있었다.
     st=-4(덧줄 2개, A3 자리)는 기둥(3.5칸)이 반드시 안쪽 덧줄(st=-2)을 지나간다. */
  ['덧줄 2개 아래 8분음표(기둥이 덧줄을 지나간다)',
    [N(-4,0.5),N(-2,0.5),N(0,0.5),N(2,0.5),N(4,0.5)], {headR:0.5}],
  ['덧줄 2개 아래 16분음표(기둥이 덧줄을 지나간다)',
    [N(-4,0.25),N(-2,0.25),N(0,0.25),N(2,0.25),N(4,0.25)], {headR:0.5}],
  /* 실측(session-handoff.md)과 같은 모양 — 같은 음(A3, 덧줄 2개)에서 길이만 바뀌어도
     덧줄 탓에 한 단계씩 짧게 읽히면 안 된다(1,0.5,0.25 가 0.5,0.25,0.125 로 밀리던 버그). */
  ['덧줄 2개 아래 같은 음, 길이만 다름(실측 재현)',
    [N(-4,1),N(-4,0.5),N(-4,0.25),N(2,1)], {headR:0.5}]
  /* ★ 위쪽 덧줄(st>=10)은 여기 넣지 않았다 — 관심 창이 위로 2.2칸까지만 열려 있어서
     (4267번째 줄 부근 `y0=top-sp*2.2`, 아래는 2.7칸) st=12 는 머리 위쪽 절반이 창 밖으로
     잘려 그 음 자체가 통째로 안 잡힌다(이 테스트가 실제로 확인했다). 이건 v73 버그와
     무관한, 원래부터 있던 위/아래 비대칭 한계다 — 사용자가 보낸 실제 버그는 "오선 **아래**
     덧줄"뿐이었으므로 여기서는 아래쪽만 재현해 둔다. */
];
